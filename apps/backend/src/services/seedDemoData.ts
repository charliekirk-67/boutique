...
export async function seedDemoData() {
  try {
    logger.info('Starting automated demo data check and seeding...');
    ...
    for (const c of customerData) {
      ...
      if (!existingOrder) {
        ...
        logger.info(`Created order ${order.invoiceNumber} (${c.stage}) for ${customer.fullName}`);
      }
    }

    logger.info('Demo data seeding completed successfully! All demo accounts have password "12345678".');
    return { success: true, message: 'All demo accounts and orders seeded successfully.' };
  } catch (err: any) {
    logger.error('Error during demo data seeding:', { metadata: { error: err.message, stack: err.stack } });
